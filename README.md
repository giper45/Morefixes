...
### Warning
Make sure to read about the `score` column of `fixes` table in the paper before querying the data. Use a threshold preferably over ~65 to reduce noise.
...
**Deactivate the venv of Prospector and switch to Morefixes venv**, then run the tool by executing `bash Code/run.sh`. This will first update the GHSA dataset in `/Code/resources/advisory-database` and download the latest CPE Dictionary from NVD, and start the whole flow mentioned in Figure 1 of the paper.
